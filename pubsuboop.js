...
class PubSub {
    ...
    unsubscribe(channel,fn) {
        ...
        subscribersListFn = subscribersListFn.filter( subscriberFn => subscriberFn != fn );
    ...
    broadcast(channel, ...param) {
        ...
        subscribersListFn.forEach( (subscriberFn) => {
            ( param.length == 0 ) ? subscriberFn.func.call(subscriberFn.ctx ) : subscriberFn.func.call(subscriberFn.ctx , ...param );
        });
    }
}